...
NOW CONFIGURED BY YAML
----------------------
All augmentation-specific parameters are read from a YAML file (see schema below).
The CLI only deals with paths/splits and the path to the YAML.

Example:
  python augment_yolo_ds.py \
    --project_dir /path/to/MedSAM \
    --data_root /path/to/yolo_split \
    --out_root /path/to/yolo_split_aug \
    --splits train val \
    --aug_yaml /path/to/augment_config.yaml

YAML SCHEMA (copy to augment_config.yaml)
-----------------------------------------
# Output/replication
out_ext: ".jpg"
multiplier: 2
...
seed: 1337
write_yaml: true   # write data.yaml under out_root

# Albumentations transform (applied to 'multiplier' copies)
transform:
  hflip_p: 0.5
  vflip_p: 0.5
  affine:
...
# Multi-scale sliding zoom sweep (whole-image)
zoom_sweep:
  enable: true
  scales: [0.35, 0.5, 0.7]
  overlap: 0.25
...
# ---------------------- Albumentations (from YAML) -----------------

def build_transform_from_yaml(cfg_t: Dict[str, Any], out_h: int, out_w: int) -> A.Compose:
    """
    Build an Albumentations transform from a YAML dict.
    Falls back to sensible defaults when fields are missing.
    """
    hflip_p = float(cfg_t.get("hflip_p", 0.5))
    vflip_p = float(cfg_t.get("vflip_p", 0.0))

    aff = cfg_t.get("affine", {}) or {}
    ...
    return A.Compose(
        [
            ...
            A.RandomResizedCrop(size=(out_h, out_w), scale=rrc_scl, ratio=rrc_rat, p=rrc_p),
            A.ColorJitter(brightness=cj_b, contrast=cj_c, saturation=cj_s, hue=cj_h, p=cj_p),
        ],
...
class AugConfig:
    ...
    # YAML-configured augmentation knobs
    out_ext: str
    multiplier: int
...
def process_one_image(
    ...
    # (B) tiling (fixed size)
    if bool(cfg.tiling.get("enable", False)):
        tiles = tile_image_and_boxes(
            im, boxes, labels,
            tile_size=int(cfg.tiling.get("tile_size", 512)),
            overlap=float(cfg.tiling.get("tile_overlap", 0.2)),
            min_tile_vis=float(cfg.tiling.get("min_tile_vis", 0.2)),
            keep_empty=bool(cfg.tiling.get("keep_empty_tiles", False)),
        )
    ...
    if bool(cfg.zoom_crops.get("enable", False)):
        zooms = make_zoom_crops(
            im, boxes, labels,
            zoom_scales=[float(s) for s in cfg.zoom_crops.get("scales", [0.5, 0.7])],
            zoom_per_obj=int(cfg.zoom_crops.get("per_obj", 1)),
            zoom_on=str(cfg.zoom_crops.get("on", "both")),
            zoom_out_size=int(cfg.zoom_crops.get("out_size", 640)),
            zoom_jitter=float(cfg.zoom_crops.get("jitter", 0.05)),
            min_vis=float(cfg.zoom_crops.get("min_vis", 0.2)),
            keep_empty=bool(cfg.zoom_crops.get("keep_empty", False)),
        )
    ...
    if bool(cfg.zoom_sweep.get("enable", False)):
        sweeps = multiscale_zoom_sweep(
            im, boxes, labels,
            scales=[float(s) for s in cfg.zoom_sweep.get("scales", [0.35, 0.5, 0.7])],
            overlap=float(cfg.zoom_sweep.get("overlap", 0.25)),
            min_vis=float(cfg.zoom_sweep.get("min_vis", 0.2)),
            keep_empty=bool(cfg.zoom_sweep.get("keep_empty", False)),
            out_size=int(cfg.zoom_sweep.get("out_size", 640)),
        )
    ...
    for k in range(int(cfg.multiplier)):
        if boxes:
        ...
        if bool(cfg.tiling.get("enable", False)) and bool(cfg.tiling.get("from_aug", False)):
            tiles = tile_image_and_boxes(
        ...
        if bool(cfg.zoom_crops.get("enable", False)) and bool(cfg.zoom_crops.get("from_aug", False)):
            zooms = make_zoom_crops(
                aug_img, aug_boxes, aug_labels,
                zoom_scales=[float(s) for s in cfg.zoom_crops.get("scales", [0.5, 0.7])],
                zoom_per_obj=int(cfg.zoom_crops.get("per_obj", 1)),
        ...
        if bool(cfg.zoom_sweep.get("enable", False)) and bool(cfg.zoom_sweep.get("from_aug", False)):
            sweeps = multiscale_zoom_sweep(
                aug_img, aug_boxes, aug_labels,
                scales=[float(s) for s in cfg.zoom_sweep.get("scales", [0.35, 0.5, 0.7])],
                overlap=float(cfg.zoom_sweep.get("overlap", 0.25)),
...
def process_split(split: str, cfg: AugConfig):
    ...
    for img_path in list_images(in_img_dir):
        ...
        if im is None:
            ...
            continue
        H, W = im.shape[:2]
        transform = build_transform_from_yaml(cfg.transform, H, W)

        # If negatives are included, ensure an empty label file exists so downstream code writes siblings consistently
        if not lbl_path.exists() and cfg.include_images_without_labels:
            (in_lbl_dir / f"{img_path.stem}.txt").write_text("")
...
def load_aug_yaml(path: Path) -> Dict[str, Any]:
    if not path.exists():
...
def parse_args() -> AugConfig:
    ap = argparse.ArgumentParser(
        description="YOLO dataset augmenter (tiles, object zooms, zoom sweeps) — augmentation knobs via YAML."
    )
    ...
    ap.add_argument("--aug_yaml", required=True,
        help="Path to YAML file with augmentation parameters.")
    args = ap.parse_args()
    ...
    aug_yaml  = _expand(args.aug_yaml)

    y = load_aug_yaml(aug_yaml)
    ...
    # Sections
    transform = dict(y.get("transform", {}))
    tiling    = dict(y.get("tiling", {}))
    zoom_crops= dict(y.get("zoom_crops", {}))
    zoom_sweep= dict(y.get("zoom_sweep", {}))
    ...
    return AugConfig(
        ...
        include_images_without_labels=include_images_without_labels,
        seed=seed,

...
def _cdr_from_masks(disc_mask: Optional[np.ndarray], cup_mask: Optional[np.ndarray]) -> Optional[float]:
    """Compute vertical CDR from masks; None if any is missing or empty."""
...
def _attach_gt_masks(images: List[Image],
    ...
    if gt_disc_root:
        disc_map = stem_map_by_first_match(gt_disc_root)
    else:
        disc_map = {}
    if gt_cup_root:
        cup_map = stem_map_by_first_match(gt_cup_root)
    else:
        cup_map = {}
...
def collect_images(args: Args) -> List[Image]:
    cfg = _CollectCfg(
        ...
        disc_masks=args.gt_disc_root if args.eval_enabled else None,  # allow GT boxes to be filled
...
def _process_one_image(
    ...
    Optional[Path], Optional[Path], Optional[Path],  # disc_mask_path, cup_mask_path, viz_path
    ...
    if cup_xyxy is None:
        ...
        return pred_cd_ratio, gt_cd_ratio, seg_disc_path, None, viz_path, disc_box_iou, None, disc_dice, None
    ...
    return pred_cd_ratio, gt_cd_ratio, seg_disc_path, seg_cup_path, viz_path, disc_box_iou, cup_box_iou, disc_dice, cup_dice
...
def _write_summary_csv(rows: List[dict], csv_path: Path) -> None:
    ...
    fields = [
        ...
        "disc_mask_path", "cup_mask_path", "viz_path",
...
def main() -> None:
    ...
    rows: List[dict] = []
    for img in images:
        (pred_cd_ratio, gt_cd_ratio, seg_disc_path, seg_cup_path, viz_path,
         disc_box_iou, cup_box_iou, disc_dice, cup_dice) = _process_one_image(
            img, bb_pred, msam, lbl_writer, args.out_disc, args.out_cup, args.out_viz,
            save_viz=args.save_viz, eval_enabled=args.eval_enabled
        )

        # YOLO label path (may or may not exist depending on writer policy)
